...
2. 全部被试使用同一版本、步骤和数据集级参数集；单独被试 ICA 诊断图仅在数据集级统计发现明确异常、判据需要复核或用户指定时生成。
...
以下内容放在质量评估报告附录的 MNE 明细中，正文引用而不重复：
...
4. **单独被试证据**：仅在数据集级统计发现明确异常、判据需要复核或用户指定时，绘制带坏道和坏段标记的连续波形、全频 PSD、电极位置以及插值前后波形；这些图不替代数据集级统计。

### 有明确观察理由的单独被试 ICA 完整诊断

仅对质量评估主模板确定的被试，使用该被试最终处理时保存的同一个 ICA 对象固定输出；没有符合条件的被试时不生成：
...
5. **处理效果**：使用 `ICA.plot_overlay()` 在同一真实伪迹时窗展示处理前后通道波形和 EEG GFP；不同被试间固定时间范围、通道、纵轴和颜色。某类伪迹不存在时标为无，不用其他类型替代。
...
- 原始与 MNE 后的常规图和运动想象任务特异图按质量评估主模板组织，不在 MNE 方法特异结果中重复。
