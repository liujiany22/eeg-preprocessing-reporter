...
1. 预处理范围包括数据集信息报告初步排除后保留的全部被试。
2. 全部被试使用同一版本、配置、步骤和判据；单独被试 ICA 诊断图仅在数据集级统计发现明确异常、判据需要复核或用户指定时生成。
...
RELAX 在预处理报告中只输出以下内容，顺序固定：

1. **执行状态**：进入流程、成功、失败的被试数；失败阶段和原因摘要；实际 RELAX 版本；实际使用的配置分支及相对本文件推荐配置的差异。
2. **实际流程与参数**：用一条按执行顺序排列的流程和一张参数表记录实际执行项。未启用的可选分支不展开。
3. **预处理结果**：按条件给出最终剔除数、保留数和保留率。

## 质量评估步骤

1. 只读取同一次 RELAX 预处理保存的坏道、ICA 权重、IC 源活动、清理状态和 Epoch 结果，不重新运行预处理。
2. 将清理前后及必要的中间 EEGLAB 数据导入 MNE，核对 IC 编号、Topography、源活动和事件样点后生成下述质量评估结果。

## 质量评估报告输出

RELAX 的实际结果放入运动想象质量评估模板第 3 节的对应方法小节。坏道、ICA、坏段、Epoch、PSD 和处理前后比较使用模板规定的常规质量评估定义；以下内容用于确定 RELAX 实际字段和解释边界：

1. **执行状态**：进入流程、成功、失败的被试数；失败阶段和原因摘要；实际 RELAX 版本；实际使用的配置分支及相对本文件推荐配置的差异。
2. **实际流程与参数**：用一条按执行顺序排列的流程和一张参数表记录实际执行项。未启用的可选分支不展开。
3. **Bad-channel 数据集级结果**：
   - 按质量评估的共同可视化规范报告逐记录计数、逐被试结果、被试等权逐通道坏道率及其置信区间；单 Session 与多 Session 使用各自规定的统计单位和区间方法。
   - Topography 和逐被试分布图按共同规范生成；逐被试图另标出 `0.10` 电极删除上限。
4. **IC 清理数据集级结果**：
   - 逐被试唯一被清理 IC 数与比例；所有成功被试均显示，数量使用从 0 开始的整数轴并标注精确值。
   - 逐被试眼动、肌电及重叠类别计数使用并列点或并列柱，不使用堆叠柱。
   - 报告唯一被清理 IC 数及比例的中位数、IQR、范围和有效被试数，并引用附录中的逐被试表。
...
以下内容放在质量评估报告附录的 RELAX 明细中，正文引用而不重复：
...
### 有明确观察理由的单独被试 ICA 完整诊断

仅对质量评估主模板确定的被试，使用该被试实际 RELAX 运行保存的同一 ICA 结果固定输出；没有符合条件的被试时不生成：
...
5. **清理效果**：在同一真实伪迹时间窗绘制清理前后通道波形和 GFP；不同被试间固定时间范围、通道、纵轴和颜色。某类伪迹没有真实命中时标为无，不以其他时段代替。
...
- 原始与 RELAX 后的常规图和运动想象任务特异图按质量评估主模板组织，不在 RELAX 方法特异结果中重复。
