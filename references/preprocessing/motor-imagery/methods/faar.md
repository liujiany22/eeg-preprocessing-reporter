...
- 只处理数据集信息报告初步排除后保留、且具有可验证运动想象事件边界的全部被试。
...
FAAR 的实际结果放入运动想象质量评估模板第 3 节的对应方法小节：

1. 执行状态、适用的被试与 Session、实际 Epoch 定义、实现来源和实际参数；未执行步骤及缺失项。
2. 全数据集 SQI、阈值和剔除率摘要，以及逐被试、Session、条件的原始、剔除和保留 Epoch 数；不得把较高 SQI 直接解释成具体伪迹类型，没有预先判据时不判断条件差异是否可接受。
3. 图按运动想象质量评估模板第 3.2.7 节的原文调研与仿制规则生成。
4. 仅在执行了解码验证时，报告与不拒绝基线的 Balanced Accuracy、F1、Precision 和跨被试离散程度；结论限定在实际验证方案内。

## 质量评估报告附录

以下内容放在质量评估报告附录的 FAAR 明细中：
...
4. 执行解码验证时放置完整指标表；不执行时不保留空白分类章节。

保留 Epoch 支持的常规图和运动想象任务特异图按质量评估主模板组织；不重建整段连续波形，也不在 FAAR 方法特异明细中重复。
