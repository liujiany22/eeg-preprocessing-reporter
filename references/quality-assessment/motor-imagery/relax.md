...
2. 全部被试使用同一 RELAX 版本、配置、电极名称映射和通道顺序。确需按设备或采集流程使用不同配置时，先分组，再在组内执行相同配置并分别统计。
3. 固定被试顺序和标准通道顺序。未采集的通道、文件缺失和处理失败分别编码，不得当作坏道或从分母中静默删除。
4. 示例被试只用于展示完整 ICA 诊断图；Bad-channel 和 IC 清理数量均以全部成功完成该流程的被试为统计单位。
