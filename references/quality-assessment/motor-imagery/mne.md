...
1. 以运动想象任务第 3 步完成被试排除后的全部被试为处理范围；不得只处理用于举例的被试。
2. 全部被试使用同一 MNE 版本、坏道指标定义、检测参数、ICA 算法、`random_state`、电极名称映射和通道顺序。确需按设备或采集流程使用不同参数时，先分组，再在组内处理并分别统计。
3. 固定被试顺序和标准通道顺序。未采集的通道、文件缺失、指标无法计算和处理失败分别编码，不得当作正常通道、坏道或从分母中静默删除。
4. 示例被试只用于完整 ICA 诊断图；Bad-channel 和 IC 删除数量均以全部成功完成该流程的被试为统计单位。
